...
describe('The share plugin', function () {
    ...
    it('enables its GActions when connected', function () {
        ...
        for (let action in remotePlugin._meta.actions)
            expect(remotePlugin.device.get_action_enabled(action)).toBe(true);
    });

    it('can send and receive files', async function () {
        spyOn(remotePlugin, '_handleFile');

        localPlugin.shareFile(Mock.getDataPath('album.png'));
    ...
    it('disables its GActions when disconnected', function () {
        ...
        for (let action in remotePlugin._meta.actions)
            expect(remotePlugin.device.get_action_enabled(action)).toBe(false);
    });
});
